(() => {
  ...
  function startCannon(){if(!cannon.ctx)return;const questions=shuffle(D.BANK);cannon.game={questions,index:0,current:questions[0],y:24,lives:5,score:0,combo:0,maxCombo:0,angle:0,aiming:false,bullet:null,rects:[],last:performance.now(),start:Date.now(),feedback:null,nextAt:0};render();requestAnimationFrame(setupCannon);}
  function runCannon(){if(!cannon.game||!cannon.ctx)return;const g=cannon.game;const now=performance.now();const dt=Math.min(40,now-g.last);g.last=now;const elapsed=Math.floor((Date.now()-g.start)/1000);if(cannon.settings.mode==='pass'&&elapsed>=120){finishCannon();return;}if(g.bullet){g.bullet.x+=g.bullet.vx*dt/16;g.bullet.y+=g.bullet.vy*dt/16;const hit=g.rects.find(r=>g.bullet.x>=r.x&&g.bullet.x<=r.x+r.w&&g.bullet.y>=r.y&&g.bullet.y<=r.y+r.h);if(hit){const index=hit.index;g.bullet=null;resolveCannonShot(index,hit.x+hit.w/2,hit.y+hit.h/2);}else if(g.bullet.x<-12||g.bullet.x>cannon.w+12||g.bullet.y<-12){g.bullet=null;cannonMessage('没有击中选项，继续调整炮口！');}}if(g.nextAt&&Date.now()>=g.nextAt){g.nextAt=0;g.index=(g.index+1)%g.questions.length;if(g.index===0)g.questions=shuffle(g.questions);g.current=g.questions[g.index];g.y=24;g.feedback=null;}if(!g.aiming&&!g.bullet&&!g.nextAt){g.y+=cannon.settings.speed*dt/16;if(g.y>cannon.h-270){g.lives--;g.combo=0;cannonMessage('题目落地，损失 1 条生命');if(g.lives<=0){finishCannon();return;}g.index=(g.index+1)%g.questions.length;g.current=g.questions[g.index];g.y=24;}}drawCannon();const clock=document.querySelector('#cClock');if(clock)clock.textContent=cannon.settings.mode==='pass'?formatTime(120-elapsed):'∞';const score=document.querySelector('#cScore'),lives=document.querySelector('#cLives'),num=document.querySelector('#cQ');if(score)score.textContent=g.score;if(lives)lives.textContent=g.lives;if(num)num.textContent=g.index+1;cannon.frame=requestAnimationFrame(runCannon);}
